    ...
    async function submit(e: SubmitEvent) {
        ...
        try {
            ...
            if (json.success && json.token) {
                // Cookie sudah di-set oleh server — beri sedikit waktu lalu redirect
                const params = new URLSearchParams(window.location.search);
                const redirectTo = params.get("redirect") ?? "/admin";
                // Gunakan setTimeout untuk memastikan cookie sudah diproses browser
                setTimeout(() => {
...
<div class="mb-8">
    ...
    <p class="text-[13px] text-ink/45 mt-1.5">
        Gunakan kredensial akun dinas Anda untuk melanjutkan.
    </p>
</div>
    ...
    <div class="space-y-1.5">
        ...
        <div class="relative">
          ...
          focus:border-green transition-colors disabled:opacity-50"
            ...
            >
                {#if showPw}
                    <Icon icon="mdi:eye-off" class="w-4.5 h-4.5" />
                {:else}
                    <Icon icon="mdi:eye" class="w-4.5 h-4.5" />
                {/if}
            </button>
        </div>
    </div>
...
<Icon icon="mdi:alert-circle" class="w-4 h-4 text-red-500 shrink-0 mt-0.5" />
    ...
    <button
        type="submit"
        disabled={loading}
        class="w-full bg-green hover:bg-green/90 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed
      text-white font-bold text-sm py-3 px-4 transition-all shadow-sm shadow-green/20
      cursor-pointer mt-2 mb-3 flex items-center justify-center gap-2"
    >
        {#if loading}
...
<!-- Back to homepage -->
<a href="/" class="text-sm text-ink/45">
    <!-- back arrow icon -->
...
<Icon icon="mdi:lock-outline" class="w-4 h-4 text-ink/25 shrink-0 mt-0.5" />
    <p class="text-[11px] text-ink/35 leading-relaxed">
        Halaman ini khusus untuk pegawai Kemenag Kota Probolinggo yang memiliki
        akun resmi. Jika mengalami masalah masuk, hubungi administrator sistem.
    </p>
</div>
