<script>
  ...
  let sorting      = $state([])
  let pagination   = $state({ pageIndex: 0, pageSize: untrack(() => initialPageSize) })
  ...
  const allColumns = $derived(
    enableSelection
      ? [{ id: '_select', header: '', enableSorting: false, size: 40 }, ...columns]
      : columns
  )
...
<div class="flex flex-col gap-0 {extraClass}">
    ...
    <table class="w-full text-sm border-collapse">
              ...
              {:else}
                <th
                  class="px-4 py-3 text-left font-semibold text-[11px] uppercase tracking-wider text-ink/40
                    {header.column.getCanSort() ? 'cursor-pointer select-none hover:text-ink/70' : ''}"
                  ...
                  {#if !header.isPlaceholder}
                    <div class="flex items-center gap-1">
                      ...
                      {#if header.column.getCanSort()}
                        <span class="inline-flex flex-col gap-px ml-0.5">
                          {#if header.column.getIsSorted() === 'asc'}
                            <svg viewBox="0 0 24 24" class="w-3.5 h-3.5 text-green"><path fill="currentColor" d="M13 20h-2V8l-5.5 5.5l-1.42-1.42L12 4.16l7.92 7.92l-1.42 1.42L13 8z"/></svg>
                          {:else if header.column.getIsSorted() === 'desc'}
                            <svg viewBox="0 0 24 24" class="w-3.5 h-3.5 text-green"><path fill="currentColor" d="M11 4h2v12l5.5-5.5l1.42 1.42L12 19.84l-7.92-7.92L5.5 10.5L11 16z"/></svg>
                          {:else}
                            <svg viewBox="0 0 24 24" class="w-3.5 h-3.5 text-ink/25"><path fill="currentColor" d="M12 18.17L8.83 15l-1.41 1.41L12 21l4.59-4.59L15.17 15M12 5.83L15.17 9l1.41-1.41L12 3L7.41 7.59L8.83 9z"/></svg>
                          {/if}
                        </span>
                      {/if}
                    </div>
      ...
      <tbody>
          ...
          {#each Array(initialPageSize) as _}
            <tr class="border-b border-black/5">
              {#each allColumns as _}
                <td class="px-4 py-3">
                  <div class="h-4 bg-black/6 rounded-md animate-pulse" style="width: {60 + Math.random() * 30}%"></div>
                </td>
              {/each}
            </tr>
          {/each}
        ...
        {:else if table.getRowModel().rows.length === 0}
          <tr>
            <td colspan={allColumns.length} class="px-4 py-16 text-center text-ink/35 text-sm">
              <svg viewBox="0 0 24 24" class="w-10 h-10 mx-auto mb-3 text-ink/20"><path fill="currentColor" d="M17 12h-5v5h5v-5M16 1v2H8V1H6v2H5c-1.11 0-1.99.9-1.99 2L3 19c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2h-1V1zm3 18H5V8h14z"/></svg>
        ...
        {:else}
          {#each table.getRowModel().rows as row}
            <tr class="border-b border-black/5 hover:bg-black/[0.018] transition-colors">
  ...
  {#if !loading}
    ...
    <div class="flex items-center justify-between px-4 py-3 border-t border-black/8 flex-wrap gap-3">

      <!-- Info + page size -->
      <div class="flex items-center gap-3 text-xs text-ink/45">
        <span>
          {from.toLocaleString('id-ID')}–{to.toLocaleString('id-ID')} dari {total.toLocaleString('id-ID')} baris
        </span>
      ...
      {#if pageCount > 1}
        <div class="flex items-center gap-1">
          ...
          >
            <svg viewBox="0 0 24 24" class="w-4 h-4"><path fill="currentColor" d="M15.41 16.58L10.83 12l4.58-4.59L14 6l-6 6l6 6z"/></svg>
          </button>

          {#each pageNumbers(currentPage, pageCount) as p}
            {#if p === '…'}
              <span class="w-8 h-8 flex items-center justify-center text-xs text-ink/30">…</span>
            {:else}
              <button
          ...
          <button
            ...
            class="w-8 h-8 flex items-center justify-center rounded-lg border border-black/10
              hover:bg-black/4 disabled:opacity-30 disabled:cursor-not-allowed transition-colors cursor-pointer"
          >
            <svg viewBox="0 0 24 24" class="w-4 h-4"><path fill="currentColor" d="M8.59 16.58L13.17 12L8.59 7.41L10 6l6 6l-6 6z"/></svg>
          </button>
